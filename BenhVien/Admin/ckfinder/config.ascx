...
<script runat="server">
    ...
    public override void SetConfig()
    {
        ...
        string root = "/Uploads/";
        if (Session["ckfunc"].ToString().Equals("tv")) { root = "/UploadTV/"; }
        var userPath = "";
        ...
        ResourceType type;
        if (Session["ckfunc"].ToString().Equals("tv"))
        {
            string idtv = Session["uID"].ToString();
            if (!idtv.Equals(""))
            {
                DataAccess.Classes.ThanhVien tv = DataAccess.Classes.ThanhVien.LayThongTinDangNhap(idtv);
                type = ResourceType.Add(idtv + "." + tv.TenDangNhap);
                type.Url = BaseUrl + idtv + "." + tv.TenDangNhap + "/";
                type.Dir = BaseDir == "" ? "" : BaseDir + idtv + "." + tv.TenDangNhap + "/";
                type.MaxSize = 5 * mb1;
